...
int mm_init(void)
{
	...
	PUT(heapStart + (1 * WSIZE), PACK(DSIZE, 1)); /* Prologue header */
	PUT(heapStart + (2 * WSIZE), PACK(DSIZE, 1)); /* Prologue footer */
	PUT(heapStart + (3 * WSIZE), PACK(0, 1));     /* Epilogue header */
...
/* $end mmfirstfit-proto */
{
	...
	for (rover = heapStart; rover < oldrover; rover = NEXT_BLKP(rover))
		if (!GET_ALLOC(HDRP(rover)) && (asize <= GET_SIZE(HDRP(rover))))
			return rover;
...
void checkheap(int verbose)
{
	...
	if ((GET_SIZE(HDRP(heapStart)) != DSIZE) || !GET_ALLOC(HDRP(heapStart)))
