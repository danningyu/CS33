...
int mm_init(void) {	
	if ((heapStart = mem_sbrk(4 * WSIZE)) == (void *)-1) //line:vm:mm:begininit
		return -1;
	...
	heapStart += (2 * WSIZE);                     //line:vm:mm:endinit  
	freeList = NULL;
	freeListSize = 0;
	return 0;
}
...
static void printBlock(void *bp) {	
	if (ISALLOCATED(bp) == 1) { //allocated case
		...
		//printf("\tallocated node, starting at 0x%x ", HEADER(bp)); //address of start of node
		//printf("\theader raw: 0x%x ", *HEADER(bp)); //raw header, LSB should be 1
		//printf("\tPayload starts at: 0x%x ", bp); //start of payload
		...
		//printf("\tfooter address: 0x%x", FOOTER(bp);
		//printf("\tfooter raw: 0x%x\n", *FOOTER(bp));
	}
	else { //free case
		//printf("\tprinting free node\n");
		//printf("\theader raw: 0x%x ", *HEADER(bp)); //raw header, LSB should be 0
		//printf("\tPayload starts at: 0x%x ", bp); //start of payload
		//printf("\tpayload size: %d ", PAYLOADSIZE(bp)); //prints size after rounding, for block size, add 16		
		//printf("\tblock size: %d ", PAYLOADSIZE(bp) + 2*WSIZE);
		//printf("\tnext ptr: 0x%x ", *NEXTPTR(bp));
		//printf("\tprev ptr: 0x%x", *PREVPTR(bp)); //prev should be a next previously printed
		//printf("\tfooter address: 0x%x", FOOTER(bp));
		//printf("\tfooter raw: 0x%x\n", *FOOTER(bp));
	}
}
...
int mm_check(void) {
	size_t* tempPtr = heapStart;
	size_t* heapEnd = mem_heap_hi();
	//loop through all blocks and check them
	int result = 1;
	for (tempPtr = heapStart; heapStart != heapEnd; heapStart = (char*)heapStart + PAYLOADSIZE(heapStart)) {
